...
function ByteArray:getBytes(__offset, __length)
	if not __offset or not __length then 
		return self._buf
	end 
	if not self._buf then 
		return table.concat({},"",__offset,__length)
	end 
...
function ByteArray:rawUnPack(__fmt)
	...
	local __next, __val = lpack.unpack(__s, __fmt)
...
function ByteArray:readDouble()
	local __, __v = lpack.unpack(self:readBuf(8), self:_getLC("d"))
	return __v
end

function ByteArray:writeDouble(__double)
	local __s = lpack.pack( self:_getLC("d"), __double)
	self:writeBuf(__s)
	return self
end

function ByteArray:readFloat()
	local __, __v = lpack.unpack(self:readBuf(4), self:_getLC("f"))
	return __v
end

function ByteArray:writeFloat(__float)
	local __s = lpack.pack( self:_getLC("f"),  __float)
	self:writeBuf(__s)
	return self
end

function ByteArray:readInt()
	local __, __v = lpack.unpack(self:readBuf(4), self:_getLC("i"))
	return __v
end

function ByteArray:writeInt(__int)
	local __s = lpack.pack( self:_getLC("i"),  __int)
	self:writeBuf(__s)
	return self
end
...
function ByteArray:writeUInt(__uint)
	local __s = lpack.pack(self:_getLC("I"), __uint)
	self:writeBuf(__s)
	return self
end

function ByteArray:readShort()
	local __, __v = lpack.unpack(self:readBuf(2), self:_getLC("h"))
	return __v
end

function ByteArray:writeShort(__short)
	local __s = lpack.pack( self:_getLC("h"),  __short)
	self:writeBuf(__s)
	return self
end
...
function ByteArray:writeUShort(__ushort)
	local __s = lpack.pack(self:_getLC("H"),  __ushort)
	self:writeBuf(__s)
	return self
end

function ByteArray:readLong()
	local __, __v = lpack.unpack(self:readBuf(8), self:_getLC("l"))
	return __v
end

function ByteArray:writeLong(__long)
	local __s = lpack.pack( self:_getLC("l"),  __long)
	self:writeBuf(__s)
	return self
end

function ByteArray:readULong()
	local __, __v = lpack.unpack(self:readBuf(4), self:_getLC("L"))
	return __v
end


function ByteArray:writeULong(__ulong)
	local __s = lpack.pack( self:_getLC("L"), __ulong)
	self:writeBuf(__s)
	return self
end

function ByteArray:readUByte()
	local __, __val = lpack.unpack(self:readRawByte(), "b")
	return __val
end

function ByteArray:writeUByte(__ubyte)
	local __s = lpack.pack("b", __ubyte)
	self:writeBuf(__s)
	return self
end

function ByteArray:readLuaNumber(__number)
	local __, __v = lpack.unpack(self:readBuf(8), self:_getLC("n"))
	return __v
end

function ByteArray:writeLuaNumber(__number)
	local __s = lpack.pack(self:_getLC("n"), __number)
	self:writeBuf(__s)
	return self
end
...
function ByteArray:readStringBytes(__len)
	...
	local __, __v = lpack.unpack(self:readBuf(__len), self:_getLC("A"..__len))
	return __v
end

function ByteArray:writeStringBytes(__string)
	local __s = lpack.pack(self:_getLC("A"), __string)
	self:writeBuf(__s)
	return self
end
...
function ByteArray:writeStringUShort(__string)
	local __s = lpack.pack(self:_getLC("P"), __string)
	self:writeBuf(__s)
	return self
end
...
function ByteArray:readChar()
	local __, __val = lpack.unpack( self:readRawByte(), "c")
	return __val
end

function ByteArray:writeChar(__char)
	self:writeRawByte(lpack.pack("c", __char))
	return self
end
