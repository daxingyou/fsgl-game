...
--[[48 神·杨玉环]]
local ssyyh = class("ssyyh", function ( params )
...
function ssyyh:_initCache()
	XTHD.createSprite("res/spine/effect/048/atk_guiji.png")
	self:getEffectSpineFromCache("res/spine/effect/048/atk0/atk0")
	self:getEffectSpineFromCache("res/spine/effect/048/atk0_1/atk0_1")
	self:getEffectSpineFromCache("res/spine/effect/048/atk0_2/atk0_2")
	for key,value in pairs(self:getSkills()) do
		if key == "skillid1" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/048/atk1/atk1")
		elseif key == "skillid2" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/048/atkd/atkd")
			XTHD.createSprite("res/spine/effect/048/atk2guiji.png")
		end
	end
end

function ssyyh:doAnimationEvent(event)
	--[[注意：
	...
	if name == BATTLE_ANIMATION_EVENT.onAtkDone then
		...
		local feibiao_sp = XTHD.createSprite("res/spine/effect/048/atk_guiji.png") 
		...
		feibiao_sp:runAction(cc.Sequence:create(cc.MoveTo:create(dt,endPos),cc.CallFunc:create(function()
			local effect_spine = self:getEffectSpineFromCache("res/spine/effect/048/atkd/atkd")
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtk1Done then
		...
		local effect_spine = self:getEffectSpineFromCache("res/spine/effect/048/atk1/atk1")
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtk2Done then
		local target = selectedTargets[1]
		-- 预留设置打击对方
		local feibiao_sp = XTHDArrow:createWithParams({fileName = "res/spine/effect/048/atk2guiji.png" , autoRotate = false})
		feibiao_sp:setScale(self:getScaleY())
		local _fireSlotNodePos = self:getSlotPositionInWorld("firePoint");
		feibiao_sp:setPosition( _fireSlotNodePos );
	...
	elseif name == "onAtk0_1Done" then
		...
		local special_effect = self:getEffectSpineFromCache("res/spine/effect/048/atk0_2/atk0_2", 1.0)
		...
		for k,target in pairs(_targetList) do
			local effect_spine = self:getEffectSpineFromCache("res/spine/effect/048/atk0/atk0")
...
function ssyyh:doAnimationStart(event)
    local winWidth = cc.Director:getInstance():getWinSize().width
    local winHeight = cc.Director:getInstance():getWinSize().height
	if event.animation == BATTLE_ANIMATION_ACTION.SUPER then
		local effect_atk0_1 = self:getEffectSpineFromCache("res/spine/effect/048/atk0_1/atk0_1", 1.0)
...
function ssyyh:create(params)
	return ssyyh.new(params)
end
