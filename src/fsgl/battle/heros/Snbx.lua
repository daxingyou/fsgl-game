...
--[[80 少年碧霄]]
local Snbx = class("Snbx", function ( params )
...
function Snbx:doAnimationEvent(event)
	--[[注意：
...
function Snbx:doAnimationStart(event)
    local winWidth = cc.Director:getInstance():getWinSize().width
    local winHeight = cc.Director:getInstance():getWinSize().height
...
function Snbx:create(params)
	return Snbx.new(params)
end
