...
local sslc = class("sslc", function ( params )
	local animal = Character:_create(params)
	return animal
end)

function sslc:_initCache()
	self:getEffectSpineFromCache("res/spine/effect/072/atk0/atk0")
...
function sslc:doAnimationEvent(event)
	...
	if name == BATTLE_ANIMATION_EVENT.onAtk0Begin then
		...
		if targets ~= nil then
			...
			if name == BATTLE_ANIMATION_EVENT.onAtk0Done then
				...
				local __sp = self:getEffectSpineFromCache("res/spine/effect/072/atk0/atk0", 1.0)
...
function sslc:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function sslc:create(params)
	return sslc.new(params)
end
