...
local SunShangXiang = class("SunShangXiang", function ( params )
...
function SunShangXiang:_initCache()
	XTHD.createSprite("res/spine/effect/025/zidanpugong.png")
	for key,value in pairs(self:getSkills()) do
		if key == "skillid1" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/025/atk1/atk1")
		elseif key == "skillid2" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/025/atk2/atk2")
			XTHD.createSprite("res/spine/effect/025/atk1/atk1zidan.png")
		end
	end
end

function SunShangXiang:doAnimationEvent(event)
	--[[注意：
	...
	if name == BATTLE_ANIMATION_EVENT.onAtkDone then
		...
		if _target_enemy then
			local _arrow = XTHD.createSprite("res/spine/effect/025/zidanpugong.png")
			...
			_arrow:runAction(cc.Sequence:create(cc.MoveTo:create(dt,endPos),cc.CallFunc:create(function()
					...
					animalClickedAnimation({
							...
							,path = "res/spine/effect/025/zidanpugong1.png"
							,attacker = self
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtk1Done then
		for k,target in pairs(_targetList) do
			local effect_spine = self:getEffectSpineFromCache("res/spine/effect/025/atk1/atk1")
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtk2Done and _targetList[1]:isAlive() == true then
		...
		local _arrow = XTHDArrow:createWithParams({fileName = "res/spine/effect/025/atk1/atk1zidan.png" , autoRotate = true})
		_arrow:setScale(self:getScaleY())
	 ...
	 --    }
		...
		_arrow:runAction(cc.Sequence:create(actionBezier,cc.CallFunc:create(function()
			local effect_spine = self:getEffectSpineFromCache("res/spine/effect/025/atk2/atk2")
			target:addNodeForSlot({node = effect_spine , slotName = "midPoint" , zorder = 10})	
			effect_spine:setAnimation(0,"atk0",false)
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtk0Done then
		for k,target in pairs(_targetList) do
			local effect_spine = self:getEffectSpineFromCache("res/spine/effect/025/atk0/atk0")
...
function SunShangXiang:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function SunShangXiang:create(params)
	return SunShangXiang.new(params)
end
