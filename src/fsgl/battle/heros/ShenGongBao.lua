...
local ShenGongBao = class("ShenGongBao", function ( params )
	local animal = Character:_create(params)
	return animal
end)

function ShenGongBao:_initCache()
...
function ShenGongBao:doAnimationEvent(event)
...
function ShenGongBao:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function ShenGongBao:create(params)
	return ShenGongBao.new(params)
end
