...
local sslzs = class("sslzs", function ( params )
	local animal = Character:_create(params)
	return animal
end)

function sslzs:_initCache()
	XTHD.createSprite("res/spine/effect/049/tu.png")
	for key,value in pairs(self:getSkills()) do
		local _level = tonumber(value.level) or 0
		if key == "skillid1" and _level > 0 then
			self:getEffectSpineFromCache("res/spine/effect/049/atk1")
			-- self:getEffectSpineFromCache("res/spine/effect/049/atk1_1")
		elseif key == "skillid2" and _level > 0 then
			self:getEffectSpineFromCache("res/spine/effect/049/atk2")
		end
	end
end

function sslzs:doAnimationEvent(event)
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtk0Done then
		...
		local eff =  sp.SkeletonAnimation:createWithBinaryFile("res/spine/049.skel", "res/spine/049.atlas", 1.0)
...
--		self:setTargetable(true)
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtk1Done then
		local _target = targets[1]
		-- local endPos = _target:getSlotPositionInWorld("root")
		-- local effect_sp = self:getEffectSpineFromCache("res/spine/effect/049/atk1_1", 1.0)
		-- if self:getFaceDirection() == BATTLE_DIRECTION.LEFT then
		...
		for k,v in pairs(targets) do
			...
			local _effSp = self:getEffectSpineFromCache("res/spine/effect/049/atk1")
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtk2Done then
		...
		local biubiu = XTHD.createSprite("res/spine/effect/049/tu.png")
		...
		biubiu:runAction(cc.Sequence:create(
			...
			cc.CallFunc:create(function()
				local effect_sp = self:getEffectSpineFromCache("res/spine/effect/049/atk2")
...
function sslzs:doSuperAnimationStart(event)
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
	XTHD.playSkillEffectAndPlaySound(_skillData["skill_pic"], _skillData["skill_tak"], self:getSide())
end

function sslzs:create(params)
	return sslzs.new(params)
end
