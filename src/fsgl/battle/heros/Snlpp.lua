--[[--95 少年柳琵琶]]
local Snlpp = class("Snlpp", function ( params )
	local animal = Character:_create(params)
	return animal
end)

function Snlpp:_initCache()
...
function Snlpp:doAnimationEvent(event)
...
function Snlpp:doAnimationStart(event)
	--[[记录当前的技能数据]]
	local _animalName = self:getNowAniName()
	local _skillData = self:getSkillByAction(_animalName)
	local targets = self:getSelectedTargets(_animalName)
...
function Snlpp:create(params)
	return Snlpp.new(params)
end
