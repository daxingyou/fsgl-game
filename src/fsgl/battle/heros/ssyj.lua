...
local ssyj = class("ssyj", function ( params )
...
function ssyj:_initCache()
	self:getEffectSpineFromCache("res/spine/effect/062/atk0/029atk01")
	-- dump(self:getSkills())
	for key,value in pairs(self:getSkills()) do
		if key == "skillid1" and tonumber(value.level) > 0 then
			-- self:getEffectSpineFromCache("res/spine/effect/062/atk1/atk1")
		elseif key == "skillid2" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/062/atk2/029atk2")
		end
	end
end

function ssyj:doAnimationEvent(event)
	...
	if name == BATTLE_ANIMATION_EVENT.onAtk0Begin then
		...
		if targets ~= nil then
			...
			if name == BATTLE_ANIMATION_EVENT.onAtkDone or name == "onAtkDone2" then
				local _spPath = "res/spine/effect/062/biao1.png"
				if name == "onAtkDone2" then
					_spPath = "res/spine/effect/062/biao2.png"
				end
				local arrow_spine = XTHD.createSprite(_spPath)
				-- self:getEffectSpineFromCache("res/spine/effect/062/atk/atk", 1.0)
			...
			elseif name == BATTLE_ANIMATION_EVENT.onAtk1Done then
				local arrow_sp = XTHD.createSprite("res/spine/effect/062/atk1/atk1.png")
				local _targetSlot = self:getSlotPositionInWorld("firePoint")
				...
				arrow_sp:runAction(cc.Sequence:create(cc.MoveTo:create(dt,endPos),cc.CallFunc:create(function()
						...
						self:doHurt({skill = _skillData,targets = targets})
						local _data = {
							file = "res/spine/effect/062/atk1/atk11", 
							name = "100", 
							startIndex = 1,
							endIndex = 12,
							perUnit = 0.1,
			...
			elseif name == BATTLE_ANIMATION_EVENT.onAtk2Done then
				...
				local effect_spine = self:getEffectSpineFromCache("res/spine/effect/062/atk2/029atk2", 1.0)
			...
			elseif name == "onAtk01Done" then
				local target = targets[1]
				local _target_pos = target:getSlotPositionInWorld("root")
				local _effect_spine = self:getEffectSpineFromCache("res/spine/effect/062/atk0/029atk01")
			...
			elseif name == BATTLE_ANIMATION_EVENT.onAtk0Done then
				...
				local effect_spine = self:getEffectSpineFromCache("res/spine/effect/062/atk0/029atk0")
...
function ssyj:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function ssyj:create(params)
	return ssyj.new(params)
end
