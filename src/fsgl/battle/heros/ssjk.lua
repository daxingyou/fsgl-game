...
local ssjk = class("ssjk", function ( params )
	local animal = Character:_create(params)
	return animal
end)

function ssjk:_initCache()
	self:getEffectSpineFromCache("res/spine/effect/067/atk0/atk0_1")
	self:getEffectSpineFromCache("res/spine/effect/067/atk0/atk0_2")
	for key,value in pairs(self:getSkills()) do
		if key == "skillid1" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/067/atk1/atk1")
		elseif key == "skillid2" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/067/atk2/atk2")
		end
	end
end

function ssjk:doAnimationEvent(event)
		...
		if targets ~= nil then
			...
			if name == BATTLE_ANIMATION_EVENT.onAtk0Done then--[[--外伤免疫buff]]
				...
				for k,target in pairs(targets) do
					local effect_spine = self:getEffectSpineFromCache("res/spine/effect/067/atk0/atk0_2")
			...
			elseif name == "onAtk1Done2" then
				for k,target in pairs(targets) do
					local effect_spine = self:getEffectSpineFromCache("res/spine/effect/067/atk1/atk1")
					target:addNodeForSlot({node = effect_spine , slotName = "midPoint" , zorder = 10})
					effect_spine:setAnimation(0,"animation",false)
					performWithDelay(effect_spine,function()
						effect_spine:removeFromParent()
			...
			elseif name == "onAtk2Done2" then	
				for k,target in pairs(targets) do
					local effect_spine = self:getEffectSpineFromCache("res/spine/effect/067/atk2/atk2")
					target:addNodeForSlot({node = effect_spine , slotName = "midPoint" , zorder = 10})
					effect_spine:setAnimation(0,"animation",false)
					performWithDelay(effect_spine,function()
						effect_spine:removeFromParent()
...
function ssjk:doSuperAnimationStart(event)
...
function ssjk:create(params)
	return ssjk.new(params)
end
