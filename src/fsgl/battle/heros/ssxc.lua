-- 神·许褚
-- id:70
local ssxc = class("ssxc", function ( params )
...
function ssxc:_initCache()
	self:getEffectSpineFromCache("res/spine/effect/070/atk0")
	self:getEffectSpineFromCache("res/spine/effect/070/atk0_1")
	for key,value in pairs(self:getSkills()) do
		local _level = tonumber(value.level) or 0
		if key == "skillid3" and _level > 0 then
			self:getEffectSpineFromCache("res/spine/effect/070/atk3")
		end
	end
end

function ssxc:doAnimationEvent(event)
	...
	if name == BATTLE_ANIMATION_EVENT.onAtk0Begin then
		...
		if targets ~= nil then
			...
			elseif name == BATTLE_ANIMATION_EVENT.onAtk0Done then
				local __sp = self:getEffectSpineFromCache("res/spine/effect/070/atk0")
			...
			elseif name == "onAtk0Done1" then
				...
				local __sp = self:getEffectSpineFromCache("res/spine/effect/070/atk0_1")
...
function ssxc:setBigToNormal( haveNoAni )
	local pX = 1
	if self:getFaceDirection() ~= BATTLE_DIRECTION.RIGHT then
		pX = -1
	end
...
function ssxc:setStatus(status)
	self._status = status
...
function ssxc:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function ssxc:create(params)
	return ssxc.new(params)
end
