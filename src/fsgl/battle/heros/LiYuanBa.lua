...
local LiYuanBa = class("LiYuanBa", function(params)
    local animal = Character:_create(params)
    return animal
end )

function LiYuanBa:_initCache()
    self._catchTb = { }
...
function LiYuanBa:doAnimationEvent(event)
...
function LiYuanBa:_catchTargets(targets)
    self._catchTb = { }
    for k, v in pairs(targets) do
        if v:isAlive() then
            local _tb = { }
...
function LiYuanBa:_selfStopSkillTarget(isNormalOver)
...
function LiYuanBa:setStatus(status)
    self._status = status
...
function LiYuanBa:doSuperAnimationStart(event)
    --
    local _animalName = self:getNowAniName()
    local _skillData = self:getSkillByAction(_animalName)
...
function LiYuanBa:create(params)
    return LiYuanBa.new(params)
end
