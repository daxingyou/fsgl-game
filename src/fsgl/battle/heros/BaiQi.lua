...
local BaiQi = class("BaiQi", function ( params )
	local animal = Character:_create(params)
	return animal
end)

function BaiQi:_initCache()
...
function BaiQi:doAnimationEvent(event)
...
function BaiQi:doAnimationStart(event)
	if event.animation=="atk0" then
        self:setImmuneControl(true)
    end
end

function BaiQi:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function BaiQi:create(params)
	return BaiQi.new(params)
end
