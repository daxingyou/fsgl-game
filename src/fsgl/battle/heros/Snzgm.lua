--少年赵公明103
local Snzgm = class("Snzgm", function ( params )
...
function Snzgm:doAnimationEvent(event)
...
function Snzgm:setNormalToBig( _skillData )
...
function Snzgm:setBigToNormal()
	self:setAttachment(self._bigSlotsName, "")
	local _node = self:getNodeForSlot("gua")
	_node:removeAllChildren()
end

function Snzgm:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function Snzgm:create(params)
	return Snzgm.new(params)
end
