--[[--50 神·芈月]]
local ssmy = class("ssmy", function ( params )
...
function ssmy:_initCache()
	self:getEffectSpineFromCache("res/spine/effect/050/atk0/atk0")
	for key,value in pairs(self:getSkills()) do
		...
		elseif key == "skillid2" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/050/atk2/atk2")
		end
	end
end

function ssmy:doAnimationEvent(event)
		...
		if targets ~= nil then
			...
			if name == BATTLE_ANIMATION_EVENT.onAtk0Done then
				...
				for k,v in pairs(targets) do 
					local effect = self:getEffectSpineFromCache("res/spine/effect/050/atk0/atk0")
			...
			elseif name == BATTLE_ANIMATION_EVENT.onAtk2Done then
				self:doHurt({skill = _skillData,targets = targets})
				for k,v in pairs(targets) do 
					local effect = self:getEffectSpineFromCache("res/spine/effect/050/atk2/atk2")
...
function ssmy:doSuperAnimationStart(event)
	-- 
...
function ssmy:create(params)
	return ssmy.new(params)
end
