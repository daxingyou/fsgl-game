--[[--神·李师师]]
local ShenLiShiShi = class("ShenLiShiShi", function ( params )
		...
		if targets ~= nil then
			...
			elseif name == BATTLE_ANIMATION_EVENT.onAtk1Done then
				for k,target in pairs(targets) do
					local effect_spine = self:getEffectSpineFromCache("res/spine/effect/047/atk1/atk1")
			...
			elseif name == BATTLE_ANIMATION_EVENT.onAtkDone then
				for k,target in pairs(targets) do
					local _arrow = XTHD.createSprite("res/spine/effect/047/biao.png")
...
function ShenLiShiShi:doAnimationStart(event)
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function ShenLiShiShi:create(params)
	return ShenLiShiShi.new(params)
end
