--[[--少年苏妲己85]]
local Snsdj = class("Snsdj", function ( params )
...
function Snsdj:doAnimationEvent(event)
	--[[注意：
...
function Snsdj:doSuperAnimationStart(event)
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
	XTHD.playSkillEffectAndPlaySound(_skillData["skill_pic"], _skillData["skill_tak"], self:getSide())
end

function Snsdj:create(params)
	return Snsdj.new(params)
end
