--闻仲
local WenZhong = class("WenZhong", function ( params )
...
function WenZhong:doAnimationEvent(event)
...
function WenZhong:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function WenZhong:_selfStopSkillTarget( ... )
	self:_resetChainNode()
	self:_resetSkillTarget()
end

function WenZhong:_checkCanCatchOne( sTarget )
...
function WenZhong:_catchOne( sTarget )
...
function WenZhong:_resetSkillTarget()
	local _target = self.mSkilltarget
...
function WenZhong:_resetChainNode( ... )
...
function WenZhong:setStatus(status)
	self._status = status
	self:_selfStopSkillTarget()
...
function WenZhong:create(params)
	return WenZhong.new(params)
end
