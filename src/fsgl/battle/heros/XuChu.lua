-- 许褚
-- id:24
local XuChu = class("XuChu", function ( params )
...
function XuChu:doAnimationEvent(event)
...
function XuChu:setBigToNormal( haveNoAni )
	local pX = 1
	if self:getFaceDirection() ~= BATTLE_DIRECTION.RIGHT then
		pX = -1
	end
...
function XuChu:setStatus(status)
	self._status = status
...
function XuChu:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function XuChu:create(params)
	return XuChu.new(params)
end
