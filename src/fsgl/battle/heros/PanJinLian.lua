--潘金莲[[--40]]
local PanJinLian = class("PanJinLian", function ( params )
...
function PanJinLian:_initCache()
...
function PanJinLian:doAnimationEvent(event)
	--[[注意：
...
function PanJinLian:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function PanJinLian:create(params)
	return PanJinLian.new(params)
end
