...
local sssgb = class("sssgb", function ( params )
	local animal = Character:_create(params)
	return animal
end)

function sssgb:_initCache()
	self:getEffectSpineFromCache("res/spine/effect/052/atk/atk")
	self:getEffectSpineFromCache("res/spine/effect/052/atk0/atk0")
	self:getEffectSpineFromCache("res/spine/effect/052/atk0/atk0_1")
	self:getEffectSpineFromCache("res/spine/effect/052/atk0/atk0_2")
	for key,value in pairs(self:getSkills()) do
		if key == "skillid2" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/052/atk2/atk2_1")
			self:getEffectSpineFromCache("res/spine/effect/052/atk2/atk2_2")
		end
	end
end
--[[--
...
function sssgb:doAnimationEvent(event)
	...
	if name == BATTLE_ANIMATION_EVENT.onAtk0Begin then
		...
		if targets ~= nil then
			...
			if name == BATTLE_ANIMATION_EVENT.onAtk0Done then
				...
				local __sp = self:getEffectSpineFromCache("res/spine/effect/052/atk0/atk0", 1.0)
			...
			elseif name == "onAtk0_1Done" then
				...
				for left_k,left_v in pairs(_leftTargets) do
					if left_v:isAlive() == true then
						...
						local _effectsp = self:getEffectSpineFromCache("res/spine/effect/052/atk0/atk0_1")
				...
				for right_k,right_v in pairs(_rightTargets) do
					if right_v:isAlive() == true then
						...
						local _targetMidWorldPos = _targetMidNode:convertToWorldSpace(cc.p(0.5, 0.5))
						local _effectsp = self:getEffectSpineFromCache("res/spine/effect/052/atk0/atk0_2")
						_effectsp:setAnimation(0,"atk0_1",false)
			...
			elseif name == BATTLE_ANIMATION_EVENT.onAtk2Done then
				...
				local effect_sp = self:getEffectSpineFromCache("res/spine/effect/052/atk2/atk2_1")
			...
			elseif name == BATTLE_ANIMATION_EVENT.onAtkDone then
				local effect_sp = self:getEffectSpineFromCache("res/spine/effect/052/atk/atk")
...
function sssgb:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function sssgb:create(params)
	return sssgb.new(params)
end
