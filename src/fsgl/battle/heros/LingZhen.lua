--[[凌振]]
local LingZhen = class("LingZhen", function ( params )
	if not params then
		params = {}
	end
	-- params.id = 17
...
function LingZhen:_initCache()
	XTHD.createSprite("res/spine/effect/017/atk2/atk2.png")
	self:getEffectSpineFromCache("res/spine/effect/017/atk0/atk0")	
	self:getEffectSpineFromCache("res/spine/effect/017/shoulei/017")	
	for key,value in pairs(self:getSkills()) do
		if key == "skillid1" and tonumber(value.level) > 0 then
			self:getEffectSpineFromCache("res/spine/effect/017/atk1/017atk1")
		elseif key == "skillid2" and tonumber(value.level) > 0 then
		end
	end
end

function LingZhen:doAnimationEvent(event)
	...
	if name == BATTLE_ANIMATION_EVENT.onAtk0Begin then
		for k,target in pairs(targets) do
			local effect_spine = self:getEffectSpineFromCache("res/spine/effect/017/atk0/atk0")
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtkDone then
		...
		if _target_enemy then
			local shoulei_sp = self:getEffectSpineFromCache("res/spine/effect/017/shoulei/017")
			-- local shoulei_sp = sp.SkeletonAnimation:create("res/spine/effect/017/shoulei/017.json", "res/spine/effect/017/shoulei/017.atlas", 1)--cc.Sprite:create("res/spine/effect/017/zhadan.png")
			--起始位置
	...
	elseif name == BATTLE_ANIMATION_EVENT.onAtk2Done then--[[--射箭]]
		for k,_target_enemy in pairs(targets) do
			local _arrow = XTHD.createSprite("res/spine/effect/017/atk2/atk2.png")
...
function LingZhen:doAnimationStart(event)
	local scene = cc.Director:getInstance():getRunningScene()
	...
	if event.animation == BATTLE_ANIMATION_ACTION.ATK1 then
		...
		local __sp = self:getEffectSpineFromCache("res/spine/effect/017/atk1/017atk1")
		-- local __sp= sp.SkeletonAnimation:create("res/spine/effect/017/atk1/017atk1.json", "res/spine/effect/017/atk1/017atk1.atlas")
...
function LingZhen:create(params)
	return LingZhen.new(params)
end
