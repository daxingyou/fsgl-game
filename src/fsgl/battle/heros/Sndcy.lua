...
local Sndcy = class("Sndcy", function ( params )
	local animal = Character:_create(params)
	return animal
end)

function Sndcy:doAnimationEvent(event)
...
function Sndcy:getChangeX( dis, startX, width )
	local distance = dis
	local winWidth = cc.Director:getInstance():getWinSize().width
	if self:getFaceDirection() == BATTLE_DIRECTION.RIGHT then
...
function Sndcy:doSuperAnimationStart(event)
	-- 
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
...
function Sndcy:create(params)
	return Sndcy.new(params)
end
