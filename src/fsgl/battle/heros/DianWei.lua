...
local DianWei = class("DianWei", function ( params )
	local animal = Character:_create(params)
	return animal
end)

function DianWei:ctor()
	self:setCannotBeMoved(true)
end

function DianWei:_initCache()
...
function DianWei:doAnimationEvent(event)
...
function DianWei:doSuperAnimationStart(event)
	local _animalName = self:getNowAniName()
    local _skillData 	  = self:getSkillByAction(_animalName)
	XTHD.playSkillEffectAndPlaySound(_skillData["skill_pic"], _skillData["skill_tak"], self:getSide())
end

function DianWei:create(params)
	return DianWei.new(params)
end
