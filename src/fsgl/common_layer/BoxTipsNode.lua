...
local BoxTipsNode = class("BoxTipsNode",function ()	
	-- return ccui.Scale9Sprite:create(cc.rect(25,25,1,1),"res/image/common/tips_bg.png")
	return ccui.Scale9Sprite:create("res/image/common/tips_bg.png")
end)

function BoxTipsNode:ctor( params )
...
function BoxTipsNode:create(params)
	return BoxTipsNode.new(params) 
end

function BoxTipsNode:init( )
	local maxWidth = 200
	local str = nil 
	----显示话
	if self._data == nil then
		...
		str = self._data[self._index]["describe"]
    end
    ...
    self:addChild(label)
    label:setPosition(15,self:getBoundingBox().height / 2)
    self._label = label
    ----倒计时
...
function BoxTipsNode:onEnter( )
	musicManager.playEffect("res/sound/sound_effect_tips.wav")
end

function BoxTipsNode:onExit( )
end

function BoxTipsNode:formatEnergyTips( )
	local str = nil
...
function BoxTipsNode:initCountDown(str, index)
