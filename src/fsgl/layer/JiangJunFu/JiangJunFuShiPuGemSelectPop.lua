...
local JiangJunFuShiPuGemSelectPop = class("JiangJunFuShiPuGemSelectPop",function ()
	return XTHD.createPopLayer()
end)

function JiangJunFuShiPuGemSelectPop:ctor(allGem,callFunc)
	self:initData(allGem)
	self:initUI(allGem,callFunc)
end
function JiangJunFuShiPuGemSelectPop:initData(data)
...
function JiangJunFuShiPuGemSelectPop:initUI(allGem,callFunc)
    ...
    local function tableCellAtIndex(table1,idx)
        ...
        for i=1,2 do
        	if idx*2+i <= #self._allGem then
			    ...
			    local gemData = gameData.getDataFromCSV("ServanEquip",{id = nowData.itemid})
			    for i=1,#XTHD.resource.AttributesNum do
			    	if gemData[XTHD.resource.AttributesName[tonumber(XTHD.resource.AttributesNum[i])]] ~= 0 then
					    ...
					    local attrPlus = XTHDLabel:createWithParams({
					        text = " +"..gemData[XTHD.resource.AttributesName[tonumber(XTHD.resource.AttributesNum[i])]].."%",
					        fontSize = 20,
...
function JiangJunFuShiPuGemSelectPop:changeTab(_type)
...
function JiangJunFuShiPuGemSelectPop:getBtnNode()
    local _btnNodeTable = {}
...
function JiangJunFuShiPuGemSelectPop:create(allGem,callFunc)
	return JiangJunFuShiPuGemSelectPop.new(allGem,callFunc)
end

return JiangJunFuShiPuGemSelectPop