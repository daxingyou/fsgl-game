...
local LiLianSaintBeastSweepLayer = class("LiLianSaintBeastSweepLayer",function ()
	local layer = XTHD.createPopLayer({isRemoveLayout = true})
	return layer
end)

function LiLianSaintBeastSweepLayer:ctor( params )
	self._callback = params.callback
	self:initUI( params )
end

function LiLianSaintBeastSweepLayer:initUI( params )
	-- dump( params, "LiLianSaintBeastSweepLayer" )
			...
			XTHDHttp:requestAsyncInGameWithParams({
                ...
                successCallback = function( finishTask )
                    ...
                    if tonumber( finishTask.result ) == 0 then
                        -- 成功获取弹窗
                        local iconData = {}
                        if finishTask.addGodStone ~= 0 then
                        	iconData[1] = {
                        		rewardtype = XTHD.resource.type.stone,
                        		num = finishTask.addGodStone,
                        	}
                        end
			            ...
			            -- 更新背包
			            gameUser.setSaintStone( finishTask.godStone )
...
function LiLianSaintBeastSweepLayer:create(params)
	return self.new(params)
end

return LiLianSaintBeastSweepLayer