...
--神器遗址开始挑战界面
local LiLianSaintBeastChapterLayer = class("LiLianSaintBeastChapterLayer",function ()
...
function LiLianSaintBeastChapterLayer:ctor(data, id, callBack)
	self.id = id
...
function LiLianSaintBeastChapterLayer:onEnter()
...
function LiLianSaintBeastChapterLayer:onCleanup()
...
function LiLianSaintBeastChapterLayer:refreshData( data )
	if data then
		self.data = data
...
function LiLianSaintBeastChapterLayer:refreshMap(id)
...
function LiLianSaintBeastChapterLayer:analysData(data)
...
function LiLianSaintBeastChapterLayer:getTagSprite( Data, sprite, callFn )
	...
	if openStatus == 2 then
		...
		chpTip:setTouchBeganCallback(function()
			chpTip:setScale(0.9)
		end)

		chpTip:setTouchMovedCallback(function()
			chpTip:setScale(1)
		end)
		chpTip:setTouchEndedCallback(function()
				chpTip:setScale(1)
				if callFn then
					callFn()
				end
...
function LiLianSaintBeastChapterLayer:getTexture(Data)
	local openStatus = self.chapterStatus[Data.instancingid]
	local chpIcon = nil
	local pType = tonumber(Data._type) or 1
	local _filePath = self._popFiles[pType]
...
function LiLianSaintBeastChapterLayer:initUI(data)
    ...
    for i=1,#self.chapterData do
		...
		local function touchCall()
			...
			elseif self.chapterStatus[nowData.instancingid] == 1 then
				...
				local chapterPop = requires("src/fsgl/layer/LiLian/LiLianSaintBeastPop.lua"):create(nowData["instancingid"],self.data, ptb)
		...
		chapter:setTouchBeganCallback(function()
			chapter:setScale(0.9)
		end)

		chapter:setTouchMovedCallback(function()
			chapter:setScale(1)
		end)
...
function LiLianSaintBeastChapterLayer:freshPercentInfo( ... )
...
function LiLianSaintBeastChapterLayer:openNext(id)
	-- data = {
	--     [1] = 1601,
	--     -- [2] = 1602,
	--     -- [3] = 1603,
...
function LiLianSaintBeastChapterLayer:getPointPos(x1,y1,x2,y2,i,all,list)
	list = list or {}
	i = i or 1
	local Vx = x1-x2
	local Vy = y1-y2
...
function LiLianSaintBeastChapterLayer:create(data,id,callfunc)
	return LiLianSaintBeastChapterLayer.new(data,id,callfunc)
end

return LiLianSaintBeastChapterLayer