...
--神器遗址选择挑战难度
local LiLianSaintBeastModeLayer = class("LiLianSaintBeastModeLayer",function ()
...
function LiLianSaintBeastModeLayer:initUI(data,_type,beast)
	self.beastData = gameData.getDataFromCSV("SuperWeaponList")
	...
	if _type == 2 then
		...
		ownBg:setPosition(consumeBg:getPositionX()+consumeBg:getBoundingBox().width+135,80)
		bg:addChild(ownBg)
		self.ownBg = ownBg
		...
		stonePlus:setPosition(consumeBg:getBoundingBox().width-5,consumeBg:getBoundingBox().height/2-1)
		consumeBg:addChild(stonePlus)

		stonePlus:setTouchEndedCallback(function ()
			print("当前通关的最大副本ID",gameUser.getInstancingId())
			if gameUser.getInstancingId() >= 45 then
				XTHD.createSaintBeastChange(self:getParent(),function ()
					self.ownNum:setString(XTHD.resource.getItemNum(consumeItem))
				end,1)
			else
				XTHDTOAST("通关第五章开启神器商店")
			end
		end)
		...
		tipStr:setPosition(30,consumeBg:getPositionY() - 30)
		bg:addChild(tipStr)
...
function LiLianSaintBeastModeLayer:refreshReward(id)
	if self.challageBtn then
		self.challageBtn:setTouchEndedCallback(function ()
			XTHDHttp:requestAsyncInGameWithParams({
                ...
                if tonumber(data.result) == 0 then
                	...
                	local LiLianSaintBeastChapterLayer = requires("src/fsgl/layer/LiLian/LiLianSaintBeastChapterLayer.lua"):create(data,id,function()
                		_par:refreshData()
                	end)
		...
		self.sweepBtn:setTouchEndedCallback(function()
			if XTHD.resource.getItemNum(self.beastData[1].consume) >= self.beastData[id].consumenum then
				local LiLianSaintBeastSweepLayer = requires("src/fsgl/layer/LiLian/LiLianSaintBeastSweepLayer.lua"):create({ consume = self.beastData[id].consumenum, own = self.beastData[id].consume, id = id, callback = function ()
					self.ownNum:setString(XTHD.resource.getItemNum(self.beastData[1].consume))
					if self.callfunc then
						self.callfunc()
					end
				end })
	        	LayerManager.addLayout( LiLianSaintBeastSweepLayer, { noHide = true } )
	        	LiLianSaintBeastSweepLayer:show()
        	else
...
function LiLianSaintBeastModeLayer:create(data,_type,beast,callfunc)
	return LiLianSaintBeastModeLayer.new(data,_type,beast,callfunc)
end

return LiLianSaintBeastModeLayer