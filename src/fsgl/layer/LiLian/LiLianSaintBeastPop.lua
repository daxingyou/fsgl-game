...
-- 准备战斗
LiLianSaintBeastPop = class("LiLianSaintBeastPop",function ()
	return XTHDPopLayer:create()
end)

function LiLianSaintBeastPop:ctor(id,data,params)
	self.itemList = {}
	self:initUI(id,data,params)
end

function LiLianSaintBeastPop:onCleanup()
	XTHD.removeEventListener(CUSTOM_EVENT.REFRESH_GODBEAST_CHAPTER)
end

function LiLianSaintBeastPop:initUI(id,data,params)
	...
	if tonumber(_type) == 3 then
	    ...
	    local openListData = gameData.getDataFromCSV("SuperWeaponList", {id = pId})
	    ...
	    local itemData = self:initData(dropList)
    ...
    elseif tonumber(_type) == 1 or tonumber(_type) == 2 then
    	...
    	local function goSelectHero( ... )
			...
			local _layer = requires("src/fsgl/layer/ChuZhan/XuanZeYingXiongCopyLayer.lua"):createWithParams({
	  			battle_type	=BattleType.GODBEASE_PVE,
	  			instancingid	=id,
	  			godBeast_data	= {
			  			["hps"] = selectData,
    ...
    elseif tonumber(_type) == 4 then
	    ...
	    healBtn:setPosition(bg:getBoundingBox().width/2+80,37.5+18)
	    bg:addChild(healBtn)
		healBtn:setScale(0.8)
	    healBtn:setTouchEndedCallback(function ()
	    	XTHDHttp:requestAsyncInGameWithParams({
	    		modules="beastEctypeZhiliao?",
	    ...
	    noHealBtn:setPosition(bg:getBoundingBox().width/2-80,37.5+18)
    	...
    	if #healList == 0 then
		    ...
		    bg:addChild(noHero)
		    noHealBtn:setPosition(bg:getBoundingBox().width/2,37.5+18)
    		...
    		self.heroList = {}
    		self.allHero = {}
    		self._healTable = CCTableView:create(cc.size(bgShadow:getBoundingBox().width,bgShadow:getBoundingBox().height-10))--761
		    self._healTable:setPosition(0,0)
		    self._healTable:setBounceable(true)
		    self._healTable:setDirection(cc.SCROLLVIEW_DIRECTION_VERTICAL) --设置横向纵向
		    self._healTable:setDelegate()
		    self._healTable:setVerticalFillOrder(cc.TABLEVIEW_FILL_TOPDOWN)
		    bgShadow:addChild(self._healTable)

		    local function cellSizeForTable(table,idx)
		        return bgShadow:getBoundingBox().width,80 -- 68
		    end
    ...
    elseif tonumber(_type) == 5 then
    	...
    	if #healList == 0 then
    		...
    		self.heroList = {}
    		self.allHero = {}
    		self._healTable = CCTableView:create(cc.size(bgShadow:getBoundingBox().width,bgShadow:getBoundingBox().height))--761
...
function LiLianSaintBeastPop:initData(data)
	-- print("数据为：")
	-- print_r(data)
	self.itemCount = 0
	local endData = {}
	for i = 1,6 do
		if data["dropprops"..i] then
			local strArr = string.split(data["dropprops"..i],'#')
			endData[tonumber(strArr[1])] = tonumber(strArr[1])
		end
	end
	for k,v in pairs(endData) do
		self.itemCount = self.itemCount + 1
	end
	-- print("长度为："..self.itemCount)
	-- print_r(endData)
	return endData
end

function LiLianSaintBeastPop:create(id,data, params)
	return LiLianSaintBeastPop.new(id,data, params)
end

return LiLianSaintBeastPop