--世界boss奖励弹窗 huangjunjian
local XiongShouLaiXiRewardPop = class("XiongShouLaiXiRewardPop",function()
    return XTHD.createPopLayer({isRemoveLayout = true})
end)
function XiongShouLaiXiRewardPop:ctor()
	self.reward_data=gameData.getDataFromCSV("MonsterAttackAward")								   
    self:init()
    self:show()
end

function XiongShouLaiXiRewardPop:init()
...
function XiongShouLaiXiRewardPop:initCellData( cell,idx )
...
function XiongShouLaiXiRewardPop:create()
    LayerManager.addShieldLayout(false, 0.1)
    local _layer = self.new()
    return _layer
end
return XiongShouLaiXiRewardPop