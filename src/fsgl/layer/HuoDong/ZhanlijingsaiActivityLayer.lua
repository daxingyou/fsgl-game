...
ZhanlijingsaiActivityLayer = class("ZhanlijingsaiActivityLayer",function(param)
	return XTHD.createPopLayer()
end)

function ZhanlijingsaiActivityLayer:ctor()
	self._ActivityBtn = {}
	self._ActivityList = {}
	self._btnBg = {}
	self._JiangLi = {}
	...
	local title = cc.Sprite:create("res/image/activities/zhanlijingsai/zhanli_title.png" )
	bg:addChild(title)
	...
	self._title = cc.Sprite:create("res/image/activities/chongbang/logo_1.png")
...
function ZhanlijingsaiActivityLayer:initBtnTableView()
...
function ZhanlijingsaiActivityLayer:initJLTableView(index)
	for i = 1,#self._btnBg do
		self._btnBg[i]:setVisible(false)
	end
	self._btnBg[index]:setVisible(true)
	if index == 3 then
		if tonumber(self._data.power) == 0 then
			self._myRankLable:setString("未上榜")
		else
			self._myRankLable:setString(tostring(self._data.myRank))
		end
		self._powerLable:setString(self._data.power)
	else
	...
	local name = string.format("res/image/activities/zhanlijingsai/logo_%d.png",index)
...
function ZhanlijingsaiActivityLayer:ChongBangRank(index)
...
function ZhanlijingsaiActivityLayer:ShowBianQiang(index)
	ClientHttp:requestAsyncInGameWithParams({
        modules = "scoreList?",
        successCallback = function( data )
            -- dump(data,"aaa")
...
function ZhanlijingsaiActivityLayer:ShowRank()
	local rankNameList = {"英雄升星","英雄进阶","英雄战力","装备升星","神器进阶"}
...
--    node:addChild(op_btn,-1)self._nowSelected
	...
	local rankName = cc.Sprite:create("res/image/activities/zhanlijingsai/ranklogo_" .. self._nowSelected ..".png")
    ...
    local function tableCellAtIndex( table, idx )
		...
		if self._nowSelected == 12 then
			typeLable:setString(rivalRankList[self._rankList[index].duanId])
		else
			--类型
			typeLable:setString(rankNameList[self._nowSelected].."：")
			local text = ""
			if self._nowSelected == 3 then
				text = self._rankList[index].power
			else
				text = self._rankList[index].data
			end
			local typeNumber =  XTHDLabel:create(text,20,"res/fonts/def.ttf")
			typeNumber:setColor(cc.c3b(107,70,43))
			typeNumber:setAnchorPoint(0,0.5)
			typeNumber:setPosition(typeLable:getContentSize().width + 5,typeNumber:getContentSize().height /2)
			typeLable:addChild(typeNumber)
...
function ZhanlijingsaiActivityLayer:updateTime()
	self:stopActionByTag(10)
...
function ZhanlijingsaiActivityLayer:create(data)
	--rank == 1 但是 list == nil 显示未上榜
    -- print("毕业典礼的数据为：")
    -- print_r(self.data)
	self._data = data
	self._rankList = data.list
	local ZhanlijingsaiActivityLayer = ZhanlijingsaiActivityLayer.new()
	if ZhanlijingsaiActivityLayer then 
		ZhanlijingsaiActivityLayer:init()
		ZhanlijingsaiActivityLayer:registerScriptHandler(function(event )
			if event == "enter" then 
				ZhanlijingsaiActivityLayer:onEnter()
			elseif event == "exit" then 
				ZhanlijingsaiActivityLayer:onExit()
			end 
		end)	
    end
	return ZhanlijingsaiActivityLayer
end

function ZhanlijingsaiActivityLayer:init( )
	self._canClick = true	
end



function ZhanlijingsaiActivityLayer:onEnter( )
...
function ZhanlijingsaiActivityLayer:onExit( ) 
end
