...
local ChongZhiFanLiLayer = class("ChongZhiFanLiLayer", function(params)
	local layer = XTHD.createSprite()
	layer:setContentSize( 640, 428 )
	return layer
end)

function ChongZhiFanLiLayer:ctor(params)
	self._exist = true
	self._ListData = {}
...
-- 
function ChongZhiFanLiLayer:onCleanup()
	self._exist = false
end

-- 处理数据
function ChongZhiFanLiLayer:initData(  )

end
-- 创建界面
function ChongZhiFanLiLayer:initUI()
	-- 标题背景
	...
	local titleLable = cc.Sprite:create("res/image/activities/newyear/chongzhifanli.png")
...
function ChongZhiFanLiLayer:updateTime()
...
function ChongZhiFanLiLayer:buildCell( cell, index, cellWidth, cellHeight )
    ...
    bg2:setPosition( cellWidth*0.5, cellHeight*0.5 )
	cell:addChild( bg2 )
	...
	elseif self._ListData[_index].state == 0 then
		...
		cell:addChild(btn_duihuan)
		btn_duihuan:setPosition(bg2:getContentSize().width*0.9 - 2,cell:getContentSize().height / 2  - 10)
		local btn_lable = XTHDLabel:create("前 往", 18,"res/fonts/def.ttf")
		btn_lable:setColor(cc.c3b(255,255,255))
		btn_duihuan:addChild(btn_lable)
		btn_lable:enableOutline(cc.c4b(103, 34, 13, 255), 2)
		btn_lable:setPosition(btn_duihuan:getContentSize().width * 0.5,btn_duihuan:getContentSize().height * 0.5)
		btn_duihuan:setTouchEndedCallback(function()
...
function ChongZhiFanLiLayer:LingQuJiangLi(index)
...
function ChongZhiFanLiLayer:updateTableViewCell()
	ClientHttp:requestAsyncInGameWithParams({
...
function ChongZhiFanLiLayer:refreshRedDot(list)
	for k, v in pairs(list) do
		if v.state == 1 then
			RedPointState[23].state = 1
			break
		else
			RedPointState[23].state = 0
		end
	end
	self.parentLayer:freshRedDot()
end

function ChongZhiFanLiLayer:SortList( _table )
...
function ChongZhiFanLiLayer:create(params)
    return self.new(params)
end

return ChongZhiFanLiLayer
