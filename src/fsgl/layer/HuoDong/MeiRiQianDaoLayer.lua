...
local MeiRiQianDaoLayer = class("MeiRiQianDaoLayer", function(params)
    return XTHD.createFunctionLayer(cc.size(856,415))
end)
-- 每日签到
function MeiRiQianDaoLayer:ctor(params)
    self.parentlayer = params.parentLayer or nil
    self:initWithData(params.httpData)
end
function MeiRiQianDaoLayer:initWithData(data)
    -- self:setOpacity(0)
    self:setCascadeOpacityEnabled(false)
    self:setCascadeColorEnabled(false)
    ...
    local _midPosX = 308
    local _upHeight = 4
    local _advertSp = XTHD.createSprite("res/image/activities/checkin/checkin_advertsp" .. month .. ".png")
    ...
    local row_height = _sizeSpr:getContentSize().height

    local tablePoint = cc.p(0,0)
    local tableWidth = row_width
    local _tablePosY = 5
    local _talbePosX = _midPosX + 5
    local tableHeight = self:getContentSize().height - 74 -_tablePosY
    self._taskTable = cc.TableView:create(cc.size(tableWidth , tableHeight))
	TableViewPlug.init(self._taskTable)
    self._taskTable:setPosition(cc.p(_talbePosX,_tablePosY))
    ...
    self:addChild(self._taskTable)
    

    --[[顶部信息]]
    local title = XTHDPushButton:createWithParams({
            normalFile = "res/image/activities/checkin/checkin_title_bg.png",
            text = LANGUAGE_KEY_TITLE_MONTH_REWARD(month)
            })
    title:setPosition(cc.p((self:getContentSize().width +_midPosX )/2 ,self:getContentSize().height - 2))
    ...
    local _labelSprite = XTHDSprite:createWithTexture(nil, cc.rect(0,0,tableWidth,30))
    _labelSprite:setOpacity(0)
    _labelSprite:setAnchorPoint(cc.p(0,0))
    _labelSprite:setPosition(cc.p(_talbePosX,self._taskTable:getBoundingBox().y+self._taskTable:getBoundingBox().height))
    ...
    label_checkin:setPosition(cc.p(360,label_all_duty:getPositionY()))
    ...
    local function tableCellAtIndex(table1,idx)
        ...
        local cell_bg = cc.Sprite:createWithTexture(nil,cc.rect(0,0,row_width,row_height))
        ...
        for i=1,per do
            if self.item_everyday[i + idx * per] ~= nil then
                ...
                item_bg:setPosition(cc.p((i * 2 - 1) * (item_bg:getContentSize().width / 2), y))
                ...
                if status == 3 then
                    ...
                    effect_bg:setPosition(cc.p(item_bg:getBoundingBox().width / 2 , item_bg:getBoundingBox().height / 2))
                ...
                item_img.data = data
                item_img:setPosition(cc.p(item_bg:getBoundingBox().width / 2 , item_bg:getBoundingBox().height / 2))
                ...
                if status == 3 then
                    ...
                    effect_circle:setPosition(cc.p(item_bg:getBoundingBox().width / 2 , item_bg:getBoundingBox().height / 2))
                ...
                local function doCheck(replenish)
                    ...
                    if replenish == 1 then
                        if self.parentlayer ~= nil then
                            self.parentlayer:addChild(confirmDialog)    
                        end
                    else
                        _rightCallback()
                    end
...
function MeiRiQianDaoLayer:create(params)
    return self.new(params)
end

return MeiRiQianDaoLayer