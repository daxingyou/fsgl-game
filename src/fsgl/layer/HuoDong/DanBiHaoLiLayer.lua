...
local DanBiHaoLiLayer = class( "DanBiHaoLiLayer", function ()
    return XTHDSprite:createWithTexture(nil,cc.rect(0,0,839,420))
end)
function DanBiHaoLiLayer:ctor(parmas)
	self:setOpacity( 0 )
...
function DanBiHaoLiLayer:initData(data)
	self._data = data
	local static = gameData.getDataFromCSV("RechargeMeetReward", {id=1} )
	self._static = static
	self._taday = {}
	for i = 1, 4 do
		self._taday[i] = {}
		self._taday[i].rewardtype = tonumber(static["type"..i])

		self._taday[i].num = static["sum"..i]
		if tonumber(self._taday[i].rewardtype) == 4 then
			self._taday[i].id = static["id"..i]
		end
	end
end
function DanBiHaoLiLayer:initUI()
	...
	-- 小浣熊
	local smallRaccoon = XTHD.createSprite( "res/image/activities/dailyRecharge/danbihaoli.png" )
	...
	local reward_title = cc.Sprite:create("res/image/activities/dailyRecharge/reward_title1.png")
	...
	--num
	local myTTF = cc.Label:createWithBMFont("res/image/activities/dailyRecharge/bigword.fnt", tonumber(self._static["param"]))
	myTTF:setAnchorPoint(cc.p(0.5,0.5))
	myTTF:setPosition(cc.p(293,106))
	...
	local tip1 = XTHDLabel:createWithParams({
		text = "可领取奖励次数："..self._data.rechargeMeetCount,
		fontSize = 22,
...
function DanBiHaoLiLayer:refreshButton()
	...
	local _selectedFile
	local flag = 0
	if tonumber(self._data.rechargeMeetCount) == 0 then
	...
	elseif tonumber(self._data.rechargeMeetCount) >= 1 then
	...
	---
	self._days:setString("可领取奖励次数："..self._data.rechargeMeetCount)
	-- self._totalIngot:setString(self._data.dayTotalPay)

end

function DanBiHaoLiLayer:getTadayReward()
	ClientHttp:requestAsyncInGameWithParams({
	    modules = "receiveRechargeMeetReward?",      --接口
	    successCallback = function(data)
	        if tonumber(data.result) == 0 then --请求成功
				...
				self._data.rechargeMeetCount = self._data.rechargeMeetCount - 1
				self:refreshButton()
	        else
...
function DanBiHaoLiLayer:create(parmas)
	return self.new(parmas)
end
function DanBiHaoLiLayer:onCleanup()

	XTHD.removeEventListener(CUSTOM_EVENT.REFRESH_RECHARGE_MSG)
end
function DanBiHaoLiLayer:onEnter( ... )

end
function DanBiHaoLiLayer:request()
	ClientHttp:requestAsyncInGameWithParams({
	    modules = "rechargeMeetRewardList?",      --接口
	    params = {}, --参数
	    successCallback = function(data)
	        if tonumber(data.result) == 0 then --请求成功
	        	self:initData(data)
...
function DanBiHaoLiLayer:onExit( ... )
end

return DanBiHaoLiLayer