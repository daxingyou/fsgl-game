...
QuanfuchongbangActivity = class("QuanfuchongbangActivity",function(param)
	return XTHD.createPopLayer()
end)

function QuanfuchongbangActivity:ctor()
	self._ActivityBtn = {}
	self._ActivityList = {}
	self._btnBg = {}
	self._JiangLi = {}
	...
	local title = cc.Sprite:create("res/image/activities/chongbang/chongbang_title.png" )
	bg:addChild(title)
...
function QuanfuchongbangActivity:initBtnTableView()
...
function QuanfuchongbangActivity:initJLTableView(index)
...
function QuanfuchongbangActivity:ShowBianQiang(index)
	ClientHttp:requestAsyncInGameWithParams({
        modules = "scoreList?",
        successCallback = function( data )
            -- dump(data,"aaa")
...
--    node:addChild(op_btn,-1)self._nowSelected
	...
	local rankName = cc.Sprite:create("res/image/activities/chongbang/ranklogo_" .. self._nowSelected ..".png")
    ...
    local function tableCellAtIndex( table, idx )
		...
		if self._nowSelected == 12 then
			...
			local text = ""
			if self._nowSelected == 3 or self._nowSelected == 4 then
				text = self._rankList[index].data.."元宝"
			elseif self._nowSelected == 14 then
...
function QuanfuchongbangActivity:updateTime()
	self:stopActionByTag(10)
...
function QuanfuchongbangActivity:create(data)
	--rank == 1 但是 list == nil 显示未上榜
    -- print("毕业典礼的数据为：")
    -- print_r(self.data)
	self._data = data
	self._rankList = data.list
	local QuanfuchongbangActivity = QuanfuchongbangActivity.new()
	if QuanfuchongbangActivity then 
		QuanfuchongbangActivity:init()
		QuanfuchongbangActivity:registerScriptHandler(function(event )
			if event == "enter" then 
				QuanfuchongbangActivity:onEnter()
			elseif event == "exit" then 
				QuanfuchongbangActivity:onExit()
			end 
		end)	
    end
	return QuanfuchongbangActivity
end

function QuanfuchongbangActivity:init( )
	self._canClick = true	
end



function QuanfuchongbangActivity:onEnter( )
...
function QuanfuchongbangActivity:onExit( ) 
end
