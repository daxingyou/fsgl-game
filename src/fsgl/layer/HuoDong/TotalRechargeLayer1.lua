...
local TotalRechargeLayer1 = class("TotalRechargeLayer1", function(params)
    return XTHDSprite:createWithTexture(nil,cc.rect(0,0,839,420))
end)

function TotalRechargeLayer1:ctor( params )
	-- dump( params, "TotalRechargeLayer1" )
	self:setOpacity(0)
	self._exist = true
	-- ui
	self._size = self:getContentSize()
	self._leftWidth = 250
	self._parent = params.parentLayer
...
-- 
function TotalRechargeLayer1:onCleanup()
	self._exist = false
...
-- 创建界面
function TotalRechargeLayer1:initUI()
	-- 背景
	...
	-- background:addChild( timeTitleLabel )
	-- -- 活动时间
	...
	-- 	fontSize  = 20,
	-- 	anchor    = cc.p( 0, 1 ),
	-- 	pos       = cc.p( timeLabel:getPositionX(), timeLabel:getPositionY() - 30 ),
	-- 	clickable = false,
	-- })
	...
	-- 	text      = LANGUAGE_TOTALRECHARGE_TEXT[2],
	-- 	fontSize  = 18,
	-- 	color     = cc.c3b( 229, 183, 47 ),
	-- 	anchor    = cc.p( 0, 1 ),
...
-- 处理数据
function TotalRechargeLayer1:initData( httpData )
	-- 状态
...
-- 创建cell
function TotalRechargeLayer1:buildCell( cell, index, cellWidth, cellHeight )
	-- cell背景
...
-- 更新cell
function TotalRechargeLayer1:updateCell( cell, index )
	index = index + 1
	-- 数据
...
-- 领取奖励
function TotalRechargeLayer1:fetchReward( yuanbao, iconData, index , btn)
...
-- 对数据进行排序
function TotalRechargeLayer1:sortData( dataTable )
	-- 分离数据
	local notFetchTable = {}
	local fetchedTable = {}
...
-- 重新请求数据
function TotalRechargeLayer1:refreshData()
	ClientHttp:requestAsyncInGameWithParams({
...
-- 刷新界面
function TotalRechargeLayer1:refreshUI()
...
function TotalRechargeLayer1:create(params)
    return self.new(params)
end

return TotalRechargeLayer1
