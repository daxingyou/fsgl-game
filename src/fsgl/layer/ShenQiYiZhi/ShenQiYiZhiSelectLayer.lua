...
function ShenQiYiZhiSelectLayer:onCleanup()
...
function ShenQiYiZhiSelectLayer:initUI()
	...
	local saintBeastIcon = XTHD.createHeaderIcon(XTHD.resource.type.servant)
	...
	self.saintBeastNum = getCommonWhiteBMFontLabel(gameUser.getServant())
	...
	saintBeastChange:setPosition(self:getBoundingBox().width-20,topDesc:getPositionY()-topDesc:getBoundingBox().height*0.7 - 10)
	self:addChild(saintBeastChange)

	saintBeastChange:setTouchEndedCallback(function ()
		XTHD.createServantChange(self,function ()
			self.saintBeastNum:setString(gameUser.getServant())
		end)
	end)
	...
	help_btn:setPosition(self:getContentSize().width - 270,self:getContentSize().height - 90)

end

function ShenQiYiZhiSelectLayer:refreshData()
...
function ShenQiYiZhiSelectLayer:refreshLayer(data)
	...
	for i=1,#data do
		...
		if data[i]._type == 1 then
			...
			nowBg:setTouchEndedCallback(function ()
				local ModeList = requires("src/fsgl/layer/ShenQiYiZhi/ShenQiYiZhiModeLayer.lua"):create(self.data,data[i]._type,i,function ()
					self:refreshData()
				end)
			...
			nowBg:setTouchEndedCallback(function ()
				XTHDHttp:requestAsyncInGameWithParams({
	                ...
	                if tonumber(net.result) == 0 then
	                	local ShenQiYiZhiChapterLayer = requires("src/fsgl/layer/ShenQiYiZhi/ShenQiYiZhiChapterLayer.lua"):create(net,data[i].id,function ()
	                		self:refreshData()
	                	end)
...
function ShenQiYiZhiSelectLayer:doCount(node)
	node:stopAllActions()
...
function ShenQiYiZhiSelectLayer:dataAnalyzer(data)
	self.data = data
...
function ShenQiYiZhiSelectLayer:create(data)
	return ShenQiYiZhiSelectLayer.new(data)
end

return ShenQiYiZhiSelectLayer