...
local ShenQiGemPop = class("ShenQiGemPop",function ()
	return XTHD.createPopLayer()
end)

function ShenQiGemPop:ctor(index,itemid)
	self:initUI(index,itemid)
end

function ShenQiGemPop:initUI(index,itemid)
	...
	local itemData = gameData.getDataFromCSV("Runelist",{id = itemid})
	...
	for i=1,#XTHD.resource.AttributesNum do
    	if itemData[XTHD.resource.AttributesName[tonumber(XTHD.resource.AttributesNum[i])]] ~= 0 then
			...
			local tip = ""
			if itemData._type > 5 then
				tip = " +"..itemData[XTHD.resource.AttributesName[tonumber(XTHD.resource.AttributesNum[i])]].."%"
			else
				tip = " +"..itemData[XTHD.resource.AttributesName[tonumber(XTHD.resource.AttributesNum[i])]]
			end
	...
	changeGem:setTouchEndedCallback(function ()
		self:getParent():showSelectGem(index)
		self:hide()
	end)
	...
	unloadGem:setTouchEndedCallback(function ()
		self:getParent():unloadGem(index)
		self:hide()
	end)
end

function ShenQiGemPop:create(index,itemid)
	return ShenQiGemPop.new(index,itemid)
end
