...
function Store.init(listener)
    ...
    if cc.storeProvider then
        print("Store.init() - store already init")
        return false
    end

    if type(listener) ~= "function" then
        print("Store.init() - invalid listener")
        return false
    end
...
function Store.failedCallback()
    if ChargeIOSPay then
        ChargeIOSPay:failedCallback()
    end
end
