...
function math.newrandomseed()
  local seed = tonumber(tostring(os.clock()):reverse():sub(1, 6))
  if seed == nil then
...
function string.isEmpty(obj)
  local empty = false
  if obj == nil or string.len(obj) < 1 then
    empty = true
  end
  return empty
end
...
local function table_ser(tablevalue, tablekey, mark, assign)
  ...
  for k, v in pairs(tablevalue) do
    ...
    elseif type(v) == "table" then
      ...
      if mark[v] then
        table.insert(assign, string.format("%s=%s", fullkey, mark[v]))
      else
...
function table.ser(var, enc)
  ...
  return enc == true and string.dump(loadstring(ret)) or ret
end
